...
def loc_loss_distill(Yteacher, Ystudent):
	b = tf.shape(Yteacher)[0]
	h = tf.shape(Yteacher)[1]
	w = tf.shape(Yteacher)[2]

	obj_probs_true = Yteacher[...,0]
	affine_pred	= Ystudent[...,1:]

	affine_pred_t = Yteacher[...,1:]
	affinex_t = tf.stack([tf.maximum(affine_pred_t[...,0],0.),affine_pred_t[...,1],affine_pred_t[...,2]],3)
	affiney_t = tf.stack([affine_pred_t[...,3],tf.maximum(affine_pred_t[...,4],0.),affine_pred_t[...,5]],3)

	v = 0.5
	base = tf.stack([[[[-v,-v,1., v,-v,1., v,v,1., -v,v,1.]]]])
	base = tf.tile(base,tf.stack([b,h,w,1]))

	pts_true = tf.zeros((b,h,w,0))

	for i in range(0, 12, 3):
		row = base[...,i:(i+3)]
		ptsx = tf.reduce_sum(affinex_t*row,3)
		ptsy = tf.reduce_sum(affiney_t*row,3)

		pts_xy = tf.stack([ptsx,ptsy],3)
		pts_true = (tf.concat([pts_true,pts_xy],3))
...
def iwpodnet_distill_loss(Yteacher, Ystudent):

	wclas = 0.5
	wloc = 0.5

	return wloc*loc_loss_distill(Yteacher, Ystudent) + wclas*clas_loss(Yteacher, Ystudent)
